...
public class ReleaseBinaryTests : IDisposable
{
    ...
    public void CurrentBuild_BasicHelpCommand_Works()
    {
        ...
        _output.WriteLine(
            $"✅ Help command works. Output preview: {result.StandardOutput.Substring(0, Math.Min(100, result.StandardOutput.Length))}...");
    }
    ...
    public void CurrentBuild_VersionFlag_Works()
    {
        ...
        _output.WriteLine(
            $"DEBUG: ExitCode={result.ExitCode}, StdoutContainsGC={result.StandardOutput.Contains("gc")}");
    ...
    private void AddFileToRepository(string repoDir, string filename, string content)
    {
        ...
        RunCommand(repoDir, "git", "-c", "user.name=Test User", "-c", "user.email=test@example.com", "commit", "-m",
            $"Add {filename}");
    }
    ...
    private class MockHttpMessageHandler : HttpMessageHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var content =
                    "{\"tag_name\": \"v1.0.0\", \"assets\": [{\"name\": \"gc-linux-x64\", \"browser_download_url\": \"https://example.com/gc-linux-x64\"}]}";
