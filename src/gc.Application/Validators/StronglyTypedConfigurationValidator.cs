...
/// <summary>
/// Strongly-typed configuration validator that uses direct property access.
/// Eliminates reflection overhead for better performance and AOT compatibility.
/// </summary>
public sealed partial class StronglyTypedConfigurationValidator
{
    /// <summary>
    /// Validates a GcConfiguration instance using direct property access.
    /// </summary>
    ...
    private static void ValidateLimits(LimitsConfiguration? limits, List<string> errors, List<string> warnings)
    {
        ...
        if (MemorySizeParser.TryParse(limits.MaxMemoryBytes, out var memBytes))
        {
    ...
    public static bool ValidateMemorySize(string? size)
    {
        if (string.IsNullOrWhiteSpace(size))
            return false;
        return MemorySizeParser.TryParse(size, out var bytes) && bytes > 0;
    }
}
