...
public static class MarkdownGeneratorExtensions
{
    public static string GenerateMarkdown(this FileContent[] contents, CliArguments args)
    {
        ...
        Array.Sort(sortedContents,
            (a, b) => string.Compare(a.Entry.Path, b.Entry.Path, StringComparison.OrdinalIgnoreCase));
    ...
    private static void WriteMarkdownToStream(TextWriter writer, FileContent[] sortedContents, CliArguments args)
    {
        ...
        foreach (var content in sortedContents)
        {
            ...
            else
            {
                ...
                try
                {
                    using var fs = new FileStream(content.Entry.Path, FileMode.Open, FileAccess.Read,
                        FileShare.ReadWrite);
                    using var reader = new StreamReader(fs, Encoding.UTF8);
        ...
        foreach (var content in sortedContents) writer.WriteLine(content.Entry.Path);

        writer.WriteLine(defaultFence);
    }

    public static (int fileCount, long totalBytes) GenerateMarkdownStreaming(this IEnumerable<FileContent> contents,
        Stream outputStream, CliArguments args)
    {
        if (contents == null) throw new ArgumentNullException(nameof(contents));
        if (outputStream == null) throw new ArgumentNullException(nameof(outputStream));
